...
class TokenizedDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        if self.tokenizer_enc:  # lb model
            outputs = self.tokenizer_enc(
                prompts,
                padding=True,
                truncation=True,
                return_tensors="pt",
        ...
        for sample in range(self.n_tasks):
            for _ in range(self.n_copies):
                yield {
                    ...
                    "mask": outputs.attention_mask[sample],
                    "task_id": sample,
    ...
    **gen_kwargs,
    ...
    for step, batch in tqdm(
        ...
        with torch.no_grad():
            ...
            masks = batch["mask"][:, : batch["input_len"]]
            if modeltype == 'seq2seq':
                if is_wrapped:
            ...
            elif modeltype == 'langbridge':

                input_ids = torch.LongTensor([tokenizer.bos_token_id])
                input_ids = input_ids.repeat(inputs.shape[0], 1).to(inputs)
        ...
        for s in generated_tokens:
            ...
            if postprocess:
                code_gens[sample].append(
                    task.postprocess_generation(gen_code, int(
                        sample) + limit_start, modeltype=modeltype)
                )
            else:
                warnings.warn(
